...
class AntigravityAuth:
    ...
    def _load_tokens(self):
        """Load saved tokens — env var takes priority over file."""
        env_token = os.getenv("ANTIGRAVITY_REFRESH_TOKEN")
        if env_token:
            self.refresh_token = env_token.strip()
            print(
                "[Antigravity] Using refresh token from ANTIGRAVITY_REFRESH_TOKEN env"
            )
            return
    ...
    def _get_auth_code(self, verifier, challenge):
        ...
        class OAuthHandler(BaseHTTPRequestHandler):
            def do_GET(self):
                ...
                self.end_headers()
                self.wfile.write(
                    b"<html><body><h1>Auth Complete</h1><p>Close this window.</p></body></html>"
                )
    ...
    def ensure_token(self):
        ...
        print(
            "[Antigravity] Browser authentication required (first time or token expired)"
        )
...
class GeminiCLIAuth:
    ...
    def _load_gemini_cli_tokens(self):
        """Load tokens — env var takes priority over gemini-cli cache file."""
        env_token = os.getenv("GEMINI_CLI_REFRESH_TOKEN")
        if env_token:
            self.refresh_token = env_token.strip()
            print(
                "[GeminiCLI Auth] Using refresh token from GEMINI_CLI_REFRESH_TOKEN env"
            )
            return
        if not os.path.exists(GEMINI_CLI_TOKEN_FILE):
            print(f"[GeminiCLI Auth] No cached tokens at {GEMINI_CLI_TOKEN_FILE}")
            print(
                f"[GeminiCLI Auth] Run 'gemini' once to authenticate first, or we'll do browser auth."
            )
            return
        try:
...
class ChatAntigravity(BaseChatModel):
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        def flush_tool_responses():
            ...
            if pending_tool_responses:
                gemini_contents.append(
                    {"role": "user", "parts": pending_tool_responses}
                )
                pending_tool_responses = []
            ...
            if isinstance(m, ToolMessage):
                ...
                if isinstance(m, SystemMessage):
                    ...
                    if hasattr(m, "tool_calls") and m.tool_calls:
                        ...
                        saved_signatures = (m.additional_kwargs or {}).get(
                            "thought_signatures", {}
                        )
        ...
        clean_model = self.model.replace("google-antigravity/", "").replace(
            "google/", ""
        )
            ...
            try:
                ...
                with urllib.request.urlopen(req) as response:
                    ...
                    try:
                        ...
                        for part in parts:
                            if "functionCall" in part:
                                ...
                                if part.get("thoughtSignature"):
                                    thought_signatures[call_id] = part[
                                        "thoughtSignature"
                                    ]
                            elif "text" in part:
                        ...
                        logger.warning(
                            f"[Antigravity] Non-standard response: {json.dumps(resp_data)[:300]}"
                        )
                        text = json.dumps(resp_data)
                ...
                if e.code == 429:
                    if attempt < MAX_RETRIES - 1:
                        wait_time = 30
                        print(
                            f"[Antigravity] Rate limited (429), waiting {wait_time}s before retry... (attempt {attempt+1}/{MAX_RETRIES})"
                        )
                ...
                if e.code == 503:
                    ...
                    if fallback:
                        print(
                            f"[Antigravity] {current_model} capacity exhausted (503), falling back to {fallback}..."
                        )
                        ...
                        print(
                            f"[Antigravity] Capacity exhausted, retrying in {wait_time}s... (attempt {attempt+1}/{MAX_RETRIES})"
                        )
                        time.sleep(wait_time)
                        continue
