# Proposal v5: RAG 효과 검증 에이전트 + 피드백 루프
...
본 제안은 회고분석 결과를 집계하여 **RAG(과거 경험 주입)가 PA 판단 품질에 실제로 기여하는가**를 검증하고, 그 결과를 시스템에 피드백하는 경로를 정의한다.
...
2. **문서별 usefulness_score 조정값** — 자동 반영 (아래 3절 참조)

---

## 3. 피드백 루프

### 3-1. RAG 문서 usefulness_score (자동화)
...
### 3-2. usefulness_score 적용: RAG 검색 파이프라인

usefulness_score가 실제 RAG 검색에 반영되는 경로를 정의한다.

#### 검색 파이프라인

```
1. 후보 생성
   키워드(FTS) top-5 + 시멘틱(ChromaDB) top-5
       ↓
2. 중복 제거 후 병합 (최대 10개, 중복 시 그 이하)
       ↓
3. usefulness_score DESC 정렬 → 상위 5개 컷 (쓰레기 제거)
       ↓
4. RRF 관련도 정렬 → 최종 3개 선별 (현재 상황 관련성 보장)
       ↓
5. PA에 주입
```
...
| 단계 | 역할 | 기준 |
|------|------|------|
| 1단계 (후보 생성) | 관련 문서를 넓게 수집 | 키워드 + 시멘틱 유사도 |
| 3단계 (usefulness 컷) | 과거에 도움이 안 됐던 문서 제거 | usefulness_score |
| 4단계 (RRF 정렬) | 현재 분석 대상과 가장 관련 높은 문서 선별 | 검색 순위 기반 관련도 |

- **유용성 → 관련성** 순서가 핵심. 유용성 필터가 쓰레기를 쳐내고, RRF가 현재 상황에 맞는 것을 골라준다.
- "관련은 있는데 과거에 도움이 안 됐던 문서"가 반복 주입되는 문제를 3단계에서 방지.
- "유용하지만 지금과 관련 없는 문서"가 뽑히는 문제는 4단계 RRF가 방지.

### 3-3. RAG 검색 기존 파라미터 (건드리지 않음)

similarity threshold 등 기존 검색 파라미터는 **고정**.  
자동 튜닝 시 되돌리기 어렵고, 현재 잘 작동하는 것을 불확실한 근거로 변경할 이유 없음.

### 3-4. RAG 경험 비중 6:4 (건드리지 않음)

현재 파이프라인 분석(60%) : RAG 경험(40%) 비중은 **고정**.  
동적 변경 시 PA 행동의 재현성이 깨지며, 회고분석 시 "이 시점에 비중이 몇이었는지"까지 추적해야 하는 복잡도가 발생.

### 3-5. PA 프롬프트 수정 (사람이 판단)
...
출력물:
  ...
  │         → 다음 RAG 검색 시 파이프라인 3단계에서 필터링에 사용
...
| usefulness_score → 검색 반영 | O (3단계 컷) | 유용성으로 필터 후 RRF로 관련성 선별 |
