...
| 2 | DB 저장 | DB 저장 우선, RAG 피드백은 추후 결정 | 데이터가 있으면 활용 방법은 나중에 정할 수 있음 |
| 3 | open/closed | 둘 다 분석, `position_status` 컬럼으로 구분 | open은 사람이 중간 점검, closed는 최종 판정 |
| 4 | 덮어쓰기 | 1 포지션 = 1 행, 재분석 시 덮어쓰기 | closed 최종 결과만 남겨야 RAG 활용 가치 있음 |
...
1. **RAG 피드백**: closed 회고분석 결과를 ChromaDB에 저장하여 PA 판단에 활용할지 여부 — DB에 데이터가 쌓인 후 판단
